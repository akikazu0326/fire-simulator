...
v2: 教育費手動入力 / 取り崩し率手動 / グラフ100歳上限 / HC初期資産入力 /
    シミュレーション期間可変（30推奨・最大50）/ 推奨シナリオ反映ボタン
"""
...
with tab_basic:
    ...
    if st.button("▶ 基本シミュレーション実行", type="primary", key="run_basic"):
        ...
        # elem/mid/high/uniはauto/private/public用のキー（手動時はダミー）
    ...
    if "basic_res" in st.session_state:
        ...
        if is_paid:
            s5.metric("現資産取り崩し可能額", f"{res['current_withdraw']}万円/月")
        else:
            s5.metric("現資産取り崩し可能額", "🔒 有料版")
...
with tab_hcsv:
    ...
    with col_p1:
        ...
        hcsv_initial = st.number_input("初期資産（万円）", 100, 999999, 10000, step=500,
                                       help="シミュレーション開始時の資産額。最終資産もこの金額ベースで表示されます。")
    ...
    if st.button("▶ シミュレーション実行", type="primary", key="run_hcsv"):
        with st.spinner(f"計算中...（{sim_years_hcsv}年間シミュレーション）"):
            if sim_method == "rolling":
                ...
                n = mc_n if is_paid else 200
                results_hc   = run_monte_carlo(mc_scenario, wr, labor_ratio, stock_w, bond_w, thr_mult, n, sim_years=sim_years_hcsv)
                results_base = run_monte_carlo(mc_scenario, wr, 1.0,         stock_w, bond_w, thr_mult, n, sim_years=sim_years_hcsv)
    ...
    if "hcsv_hc" in st.session_state:
        ...
        # 最終資産を初期資産ベースに換算
        median_final_man = s_hc["median_final"] * h_init  # 万円
        ...
        for col, label, val, color in [
            ...
            (k3, f"中央値最終資産",   f"{median_final_man:,.0f}万円",       C["blue"]),
        ...
        # パーセンタイル帯グラフ（万円表示）
        scale_man = h_init / INITIAL_ASSET  # 論文の1,000,000→万円換算
